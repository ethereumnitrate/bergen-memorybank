...
- Application-owned code and storage do not create or use cookies, browser storage, remote data requests, telemetry, or a Canvas API. Google-hosted authentication and access-control behavior is outside the application's proof boundary and must be reviewed in the authorized deployment.
- The conservative local check inspects raw text before JSON parsing and checks it again before structural validation or package generation. It refuses explicit real-student-data markings; likely email addresses, identifiers, and filenames; `Student:` or `Learner:` labels with optional `name`; student-name-plus-course-context combinations with or without linking verbs; named possessive and `grade for` forms; possessive or person-named accommodation wording; person-linked disability, diagnosis, and health disclosures; and identifying combinations of details. Refusal messages name only the detected category, the rejected text is cleared from the page, and the safe alternative is a de-identified Class Learning Snapshot or reviewed class-level course content. Detection is deliberately bounded to student-specific context so ordinary class-level or course-subject wording can continue; uncertain content can always use manual Canvas entry. This check does not prove that content is de-identified, so faculty confirmation remains required.
